function [C_N_f2, C_P_f2, C_A_f2] = Fins_Stage_2_Coefficients(d_bt, AoA, Re)
n = 2; %number of fins
dn = d_bt; %diameter at base of nosecone
df = d_bt; %diameter of body tube at fins
ls = 0.06; %fin span
lr = .11; %fin root chord
lt = .06; %fin tip-chord
...
C_N_f2 = ((AoA).*K_fb.*(2.*n.*(ls./dn).^2)./(1+sqrt(1+((2.*lm)./(lr+lt)).^2))); %coefficient of stability (2 fins because this number will be used for both directions

%Center of Pressure

%Xf = .83; %tip of nosecone to fin leading edge
...
C_D_f2 = C_D_0 + C_D_i + C_D_AoA; %Total drag coefficient

%Axial Coefficient
C_A_f2 = ((C_D_f2.*cos(AoA)-(1./2).*C_N_f2.*sin(2.*AoA))./(1-sin(AoA).^2));
end