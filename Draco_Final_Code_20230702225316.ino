...
const uint32_t LOG_INTERVAL_USEC = 30000;
...
#endif  // __AVR_ATmega328P__
//------------------------------------------------------------------------------
const float seapress = 1010.4984;  //pressure (hPa) at sea level in day in your region
const float act_alt;
...
unsigned long First_Stage_Burnout;
unsigned long Second_Motor_Ignite;
...
void logRecord(data_t* data, uint16_t overrun) {
  if (overrun) {
    ...
    timeInt = data->time - timePrev;


      if (data->altitude > maxAltitude && Launch_Time != 0) { 
         maxAltitude = data->altitude; 
         apogeeTime = data->time;
      }
      
      if (abs(data->gx) > 4000 || abs(data->gy) > 4000 || abs(data->gz) > 4000 && Launch_Time != 0) {
        if (unstableTime = 0) {
          unstableTime = data->time;
        }
      } else {
        unstableTime = 0;
      }

      stopRecording = (Launch_Time != 0 && mpu.getZeroMotionDetected() || ((data->time - Launch_Time) > 1800000000));
        ...
        case 2:  //First_Motor
          {
            if (data->az) {
              State_Machine = 3;
              First_Stage_Burnout = data->time;
            }

          }
        break;
        case 3:  //First_Motor_Burnout
          {
            if (data->az > 0) {
              State_Machine = 4;
              Second_Motor_Ignite = data->time;
            }
          }
        break;
        case 4: //Second Motor
...
void printRecord(Print* pr, data_t* data) {
  ...
  if (!data) {
    ...
    pr->print(F("rec#"));
    pr->print(F(",State, Time,Acc X,Acc Y, Acc Z,Gyro X, Gyro Y, Gyro Z, Altitude, lat, lng"));
    pr->println();
    nr = 0;
    return;
  }
...
void setup() {
  ...
  //UKF Setup
    ...
    if (mpu.testConnection()) {
      ...
      mpu.setFreefallDetectionThreshold(60);
      Serial.println(mpu.getFreefallDetectionThreshold());
      mpu.setFreefallDetectionDuration(255);
    ...
    float off_gyro_y = gy;
    float off_gyro_z = gz;
    float altitude_test = bmp.readAltitude(seapress) + 6.25;
